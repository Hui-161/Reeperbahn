...
const ctx = {
  ...
  seenShow: new Set(['1', '2', '3']),       // Alpha zweimal, Beta einmal, Gamma ohne Termin
...
ok('Drei Konzerte abgehakt', st.konzerte === 3, String(st.konzerte));
...
ok('Minuten: 45 + 30 + 30 (Ersatz)', st.minuten === 105 && st.geschaetzt === 1,
   `${st.minuten}, geschaetzt ${st.geschaetzt}`);
ok('Formatiert als 1 h 45 min', st.minutenText === '1 h 45 min', st.minutenText);
...
ok('Tage: Do 2 Konzerte, Fr 1', st.tage.length === 2 && st.tage[0].wd === 'Do'
   && st.tage[0].konzerte === 2 && st.tage[1].konzerte === 1,
...
ok('Team: L hat Beta und Gamma (ueber Auftritt 4) gesehen, beide: 2',
   st.team && st.team.gesehen === 2 && st.team.beide.length === 2, JSON.stringify(st.team));
...
ok('Spielorte: Molotow 2 Konzerte vor Docks 1', orte[0].venue.n === 'Molotow' && orte[0].konzerte === 2
   && orte[1].venue.n === 'Docks');
ok('Molotow: Durchschnitt aus 1 und 2,5 = 1,8 (gerundet)', orte[0].avg === 1.8, String(orte[0].avg));
ok('Molotow: 45 + 30 Minuten', orte[0].minuten === 75);
...
const st3 = A.stats({ ...ctx, seen: new Set(), seenShow: new Set(['5']) });
...
ok('Konzerte: Kopf + 3 Zeilen', sheets[0].rows.length === 4);
...
ok('Notiz mit einfachen Leerzeichen', sheets[0].rows[2][11] === 'laut und gut');
